...
$(document).ready(() => {
    ...
    $('#save-info').on('click',(evt) => {
        ...
        if (isValid) {
            ...
            const profile = [];
            //add values to array
            profile['email-address'] = emailAddress;
            profile['phone-number'] = phoneNumber;
            profile['zip-code'] = zipCode;
            profile['date-of-birth'] = dateOfBirth;
            profile['credit-card-number'] = creditCard;
            profile['expiration-date'] = expirationDate;
